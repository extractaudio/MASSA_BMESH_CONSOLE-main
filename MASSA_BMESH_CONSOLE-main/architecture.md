# Architectural Standards: Golden Rules

## Residential Circulation

- **Rule:** Residential Hallway Width
- **Value:** 0.91 m (Min), 1.07-1.22 m (Ideal)
- **Reasoning:** Minimum required by code (36"); wider allows furniture movement and two people to pass comfortably.
- **Constraint:** Strict Min: 0.91m

- **Rule:** ADA Ramp Slope
- **Value:** 1:12 Ratio
- **Reasoning:** Max slope for wheelchair safety (1 inch rise per 12 inches run).
- **Constraint:** Max Slope: 4.8 degrees

- **Rule:** ADA Ramp Landing Length
- **Value:** 1.52 m
- **Reasoning:** Minimum length for resting or turning a wheelchair.
- **Constraint:** Min: 60 inches (1.52m)

## Kitchens (NKBA Guidelines)

- **Rule:** Work Triangle Leg
- **Value:** 1.22 m - 2.74 m
- **Reasoning:** To ensure appliances are within efficient reach but not too close to cramp movement.
- **Constraint:** Min: 1.22m, Max: 2.74m

- **Rule:** Work Triangle Perimeter
- **Value:** 4.0 m - 7.9 m
- **Reasoning:** Total travel distance efficiency.
- **Constraint:** Max: 7.9m

- **Rule:** Work Aisle Width (Single Cook)
- **Value:** 1.07 m
- **Reasoning:** Minimum width for one person to work comfortably.
- **Constraint:** Min: 1.07m

- **Rule:** Work Aisle Width (Multiple Cooks)
- **Value:** 1.22 m
- **Reasoning:** Allows two people to work back-to-back or pass each other.
- **Constraint:** Min: 1.22m

- **Rule:** Walkway Width
- **Value:** 0.91 m
- **Reasoning:** Clearance for traffic flow through a kitchen (no cooking).
- **Constraint:** Min: 0.91m

- **Rule:** Base Counter Height
- **Value:** 0.91 m
- **Reasoning:** Standard ergonomic height for standing work.
- **Constraint:** Fixed Standard: 36 inches (0.91m)

- **Rule:** Upper Cabinet Clearance
- **Value:** 0.46 m
- **Reasoning:** Space for countertop appliances and workspace visibility.
- **Constraint:** Standard: 18 inches (0.46m)

- **Rule:** Island Seating Overhang
- **Value:** 0.25 m - 0.38 m
- **Reasoning:** Leg clearance for comfortable seating at counter height.
- **Constraint:** Min: 10 inches (0.25m), Ideal: 15 inches (0.38m)

- **Rule:** Kitchen Pendant Height (Above Island)
- **Value:** 0.76 m - 0.91 m
- **Reasoning:** Clear sightlines while providing task lighting.
- **Constraint:** Range: 30-36 inches above counter

- **Rule:** Kitchen Toe Kick Dimensions
- **Value:** 8.9 cm H x 7.6 cm D
- **Reasoning:** Ergonomic recess for feet while standing at counter.
- **Constraint:** Standard: 3.5" H x 3" D

## Bathrooms

- **Rule:** Toilet Front Clearance
- **Value:** 0.53 m
- **Reasoning:** Legroom and space to stand up.
- **Constraint:** Strict Min: 0.53m (Code), Recommended: 0.61m

- **Rule:** Toilet Side Clearance (Center to Wall)
- **Value:** 0.38 m
- **Reasoning:** Elbow room and access.
- **Constraint:** Strict Min: 0.38m

- **Rule:** Double Vanity Center-to-Center
- **Value:** 0.76 m
- **Reasoning:** Minimum distance between two sinks to allow simultaneous use.
- **Constraint:** Min: 0.76m

- **Rule:** Shower Dimensions
- **Value:** 0.76 m x 0.76 m (Min), 0.91 m x 0.91 m (Ideal)
- **Reasoning:** Minimum space to stand and turn; larger for elbow room.
- **Constraint:** Strict Min: 30x30 inches (0.76m)

- **Rule:** Shower Bench Dimensions
- **Value:** 0.38 m x 0.51 m (Min)
- **Reasoning:** ADA standard for safe transfer and seating.
- **Constraint:** Min Depth: 15-16 inches (0.38-0.41m)

- **Rule:** Shower Head Height
- **Value:** 2.03 m - 2.13 m
- **Reasoning:** Height to accommodate taller users; ADA range lower (38-48").
- **Constraint:** Standard: 80-84 inches (2.0-2.1m)

- **Rule:** Grab Bar Height (Shower)
- **Value:** 0.84 m - 0.91 m
- **Reasoning:** Accessible height for support and balance.
- **Constraint:** Standard: 33-36 inches (0.84-0.91m)

- **Rule:** Towel Ring Height
- **Value:** 1.22 m - 1.32 m
- **Reasoning:** Accessible reach near vanity; often 20" above counter.
- **Constraint:** Standard: 48-52 inches (1.2-1.3m)

- **Rule:** Robe Hook Height
- **Value:** 1.52 m - 1.78 m
- **Reasoning:** High enough for long robes; 66" ideal.
- **Constraint:** Standard: 60-70 inches (1.5-1.7m)

- **Rule:** ADA Toilet Stall Width
- **Value:** 1.52 m
- **Reasoning:** Minimum width for wheelchair maneuverability.
- **Constraint:** Strict Min: 60 inches (1.52m)

- **Rule:** ADA Toilet Stall Depth (Wall-Hung)
- **Value:** 1.42 m
- **Reasoning:** Minimum depth for wheelchair clearance with wall-hung toilet.
- **Constraint:** Strict Min: 56 inches (1.42m)

## Staircases

- **Rule:** Riser Height (Residential)
- **Value:** 0.197 m
- **Reasoning:** Maximum step height ensures safety and regularity.
- **Constraint:** Max: 7.75 inches (0.197m)

- **Rule:** Tread Depth
- **Value:** 0.254 m
- **Reasoning:** Provides sufficient foothold for safe descent.
- **Constraint:** Min: 10 inches (0.254m)

- **Rule:** Handrail Height
- **Value:** 0.86 m - 0.97 m
- **Reasoning:** Natural height for gripping while walking.
- **Constraint:** Range: 34-38 inches

- **Rule:** Headroom Clearance
- **Value:** 2.03 m
- **Reasoning:** Prevents head impact while using stairs.
- **Constraint:** Min: 80 inches (2.03m)

## Closets & Storage

- **Rule:** Hanger Depth (Standard)
- **Value:** 0.61 m
- **Reasoning:** Accommodates standard hangers without clothing touching the back wall.
- **Constraint:** Min: 0.61m (24 inches)

- **Rule:** Walk-in Closet Width (Single Side)
- **Value:** 1.22 m - 1.52 m
- **Reasoning:** Allows for one row of hanging (0.61m) plus walking space.
- **Constraint:** Min: 1.22m

- **Rule:** Walk-in Closet Width (Double Side)
- **Value:** 1.83 m
- **Reasoning:** Two rows of hanging (1.22m total) plus a 0.61m central aisle.
- **Constraint:** Min: 1.83m

- **Rule:** Closet Rod Height (Single)
- **Value:** 1.63 m - 1.68 m
- **Reasoning:** Accommodates long garments (dresses/coats).
- **Constraint:** Standard: 64-66 inches (1.63-1.68m)

- **Rule:** Closet Rod Heights (Double)
- **Value:** 2.03 m (Top) / 1.02 m (Bottom)
- **Reasoning:** Maximizes vertical space for shirts/pants.
- **Constraint:** Standard: 80" Top, 40" Bottom

- **Rule:** Bookshelf Spacing (Vertical)
- **Value:** 0.20 m - 0.30 m
- **Reasoning:** Fits standard hardcovers and paperbacks.
- **Constraint:** Standard: 8-12 inches

## Media & Living

- **Rule:** 4K TV Viewing Distance
- **Value:** 1.2x - 1.5x Diagonal Screen Size
- **Reasoning:** Closer sitting distance possible due to high pixel density.
- **Constraint:** Optimal Range

- **Rule:** 1080p TV Viewing Distance
- **Value:** 1.5x - 2.5x Diagonal Screen Size
- **Reasoning:** Further distance required to avoid seeing pixel grid.
- **Constraint:** Optimal Range

- **Rule:** Coffee Table Distance
- **Value:** 0.36 m - 0.46 m
- **Reasoning:** Legroom and reachability from sofa.
- **Constraint:** Range: 14-18 inches (0.36-0.46m)

- **Rule:** Picture Hanging Height
- **Value:** 1.45 m - 1.52 m
- **Reasoning:** Eye level for gallery viewing (center of art).
- **Constraint:** Center: 57-60 inches (1.45-1.52m)

- **Rule:** Curtain Rod Height
- **Value:** 0.10 m - 0.15 m (Above Window)
- **Reasoning:** Creates illusion of height; or mount at ceiling.
- **Constraint:** Min: 4-6 inches above trim

- **Rule:** Curtain Rod Width Extension
- **Value:** 0.15 m - 0.25 m (Per Side)
- **Reasoning:** Allows curtains to stack clear of the glass (max light).
- **Constraint:** Min: 6-10 inches beyond trim

## Dining & Seating

- **Rule:** Table Width Per Person
- **Value:** 0.61 m
- **Reasoning:** Minimum elbow room for eating comfortably.
- **Constraint:** Min: 0.61m (24 inches)

- **Rule:** Chair Push-Back Clearance
- **Value:** 0.61 m (Min), 0.76 m (Comfort)
- **Reasoning:** Space required to slide chair back and stand up.
- **Constraint:** Min: 0.61m

- **Rule:** Dining Passage Clearance (Behind Seated)
- **Value:** 0.91 m - 1.22 m
- **Reasoning:** Allows a person to traffic behind a seated diner (service or exit).
- **Constraint:** Min: 0.91m

- **Rule:** Dining Chandelier Height (Above Table)
- **Value:** 0.76 m - 0.91 m
- **Reasoning:** Clear sightlines for seated diners; ambience.
- **Constraint:** Range: 30-36 inches above table

## Bedroom

- **Rule:** Bed Length (Standard Queen/King)
- **Value:** 2.03 m
- **Reasoning:** Standard mattress length (80 inches).
- **Constraint:** Fixed Standard

- **Rule:** Bed Side Clearance
- **Value:** 0.61 m - 0.91 m
- **Reasoning:** Space to walk, make the bed, and access nightstands.
- **Constraint:** Min: 0.61m

- **Rule:** Dresser Drawer Clearance
- **Value:** 0.91 m
- **Reasoning:** Space to stand in front of open drawers.
- **Constraint:** Min: 0.91m

## Doors & Electrical

- **Rule:** Interior Door Width
- **Value:** 0.76 m - 0.81 m
- **Reasoning:** Standard passage widths; 0.91m (36") required for accessibility.
- **Constraint:** Typical: 30-32 inches

- **Rule:** Light Switch Height (Center)
- **Value:** 1.22 m
- **Reasoning:** Ergonomic height for standing adults; accessible range.
- **Constraint:** Standard: 48 inches (1.22m)

- **Rule:** Electrical Outlet Height (Bottom)
- **Value:** 0.30 m - 0.41 m
- **Reasoning:** Low enough to be discreet, high enough for plug access.
- **Constraint:** Standard: 12-16 inches

- **Rule:** Thermostat Mounting Height
- **Value:** 1.32 m - 1.52 m
- **Reasoning:** Eye level for reading; 48" (1.22m) max for ADA.
- **Constraint:** Standard: 52-60 inches (1.3-1.5m)

- **Rule:** Door Handle Height
- **Value:** 0.91 m
- **Reasoning:** Ergonomic hand height.
- **Constraint:** Standard: 36 inches (0.91m)

- **Rule:** Deadbolt Height
- **Value:** 1.02 m - 1.22 m
- **Reasoning:** 6-12 inches above the door handle.
- **Constraint:** Range: 40-48 inches (1.0-1.2m)

- **Rule:** Peephole Height
- **Value:** 1.47 m - 1.68 m
- **Reasoning:** Average adult eye level; lower (1.09m) for accessibility.
- **Constraint:** Standard: 58-66 inches (1.4-1.6m)

## Works Surfaces & Utility

- **Rule:** Office Desk Depth
- **Value:** 0.61 m - 0.76 m
- **Reasoning:** Distance for monitors and writing space.
- **Constraint:** Min: 24 inches (0.61m)

- **Rule:** Laundry Appliance Depth (Front Load)
- **Value:** 0.81 m - 0.86 m
- **Reasoning:** Modern front-loaders are deep; plus 6" rear clearance.
- **Constraint:** Machine Depth: ~32-34 inches

- **Rule:** Single Car Garage Dimensions
- **Value:** 3.66 m x 6.10 m (Min)
- **Reasoning:** 12' width allows for door opening; 20' length for vehicle + walking.
- **Constraint:** Min: 12x20 feet

## Fixtures & Safety

- **Rule:** Toilet Paper Holder Height
- **Value:** 0.66 m
- **Reasoning:** Ergonomic reach from seated position (center of holder).
- **Constraint:** Standard: 26 inches (0.66m)

- **Rule:** Toilet Paper Holder Distance
- **Value:** 0.20 m - 0.30 m
- **Reasoning:** Forward reach from toilet bowl front.
- **Constraint:** Range: 8-12 inches (0.20-0.30m)

- **Rule:** Towel Bar Height
- **Value:** 1.22 m
- **Reasoning:** Standard reach height and clearance for towels.
- **Constraint:** Standard: 48 inches (1.22m)

- **Rule:** Egress Window Sill Height
- **Value:** 1.12 m
- **Reasoning:** Maximum height to allow safe escape in emergency.
- **Constraint:** Strict Max: 44 inches (1.12m)

- **Rule:** Wall Sconce Height
- **Value:** 1.52 m - 1.83 m
- **Reasoning:** Eye level for ambient lighting; commonly flanking mirrors.
- **Constraint:** Range: 60-72 inches (1.5-1.8m)

- **Rule:** Drinking Fountain Spout Height
- **Value:** 0.91 m
- **Reasoning:** Max height for wheelchair accessibility.
- **Constraint:** Max: 36 inches (0.91m)

## Warehouse & Industrial

- **Rule:** Standard Loading Dock Height
- **Value:** 1.22 m - 1.32 m
- **Reasoning:** Matches standard freight carrier truck bed heights.
- **Constraint:** Standard: 48-52 inches (1.22-1.32m)

- **Rule:** Pallet Rack Aisle Width (Wide)
- **Value:** 3.35 m - 3.96 m
- **Reasoning:** Allows clearance for standard counterbalanced sit-down forklifts.
- **Constraint:** Min: 11-13 feet (3.35-3.96m)

- **Rule:** Pallet Rack Aisle Width (Narrow)
- **Value:** 2.44 m - 3.05 m
- **Reasoning:** Sized for reach trucks and stand-up forklifts.
- **Constraint:** Range: 8-10 feet (2.44-3.05m)

- **Rule:** Industrial Walkway Width (General)
- **Value:** 1.22 m
- **Reasoning:** OSHA recommendation: 3 feet wider than equipment or min 4 feet.
- **Constraint:** Min: 4 feet (1.22m)

- **Rule:** Shipping Container Dimensions (20ft)
- **Value:** 6.05 m L x 2.43 m W
- **Reasoning:** Standard TEU ISO dimensions.
- **Constraint:** Fixed Standard

- **Rule:** Shipping Container Dimensions (40ft)
- **Value:** 12.19 m L x 2.43 m W
- **Reasoning:** Standard FEU ISO dimensions.
- **Constraint:** Fixed Standard

- **Rule:** Industrial Egress Route Width
- **Value:** 0.71 m
- **Reasoning:** OSHA minimum for emergency exit access.
- **Constraint:** Strict Min: 28 inches (0.71m)

- **Rule:** Industrial Catwalk Width
- **Value:** 0.46 m
- **Reasoning:** OSHA minimum for safe passage on elevated runways.
- **Constraint:** Strict Min: 18 inches (0.46m)

- **Rule:** Warehouse Clear Height
- **Value:** Varies (e.g., 6m - 12m)
- **Reasoning:** Vertical distance from floor to lowest ceiling obstruction (sprinklers/beams).
- **Constraint:** Critical Vertical Limit

- **Rule:** Mezzanine Headroom Clearance
- **Value:** 2.13 m
- **Reasoning:** Minimum clear height above and below mezzanine deck.
- **Constraint:** Min: 7 feet (2.13m)

## Commercial & Retail

- **Rule:** Standard Office Cubicle
- **Value:** 1.83 m x 1.83 m
- **Reasoning:** Common "6x6" layout for individual workstations.
- **Constraint:** Standard: 6x6 feet

- **Rule:** Conference Table Clearance
- **Value:** 1.22 m
- **Reasoning:** Minimum perimeter space for walking behind seated participants.
- **Constraint:** Min: 48 inches (1.22m)

- **Rule:** Retail Aisle Width (ADA)
- **Value:** 0.91 m (Min), 1.52 m (Passing)
- **Reasoning:** 36" for single wheelchair; 60" for two to pass.
- **Constraint:** Strict Min: 36 inches (0.91m)

- **Rule:** Commercial Bar Height (ADA)
- **Value:** 0.86 m
- **Reasoning:** Maximum height for accessible bar counter section.
- **Constraint:** Max: 34 inches (0.86m)

- **Rule:** Commercial Bar Leg Clearance
- **Value:** 0.48 m
- **Reasoning:** Knee depth required for inclusivity at accessible bar.
- **Constraint:** Min: 19 inches (0.48m)

- **Rule:** Fitting Room Turning Space
- **Value:** 1.52 m Diameter
- **Reasoning:** ADA requirement for 360-degree wheelchair turn.
- **Constraint:** Min: 60 inches (1.52m)

- **Rule:** Fitting Room Bench Size
- **Value:** 1.07 m L x 0.51 m D
- **Reasoning:** Accessible bench requirement for transfer.
- **Constraint:** Min: 42" x 20" (1.07x0.51m)

- **Rule:** Service Counter Height
- **Value:** 0.91 m
- **Reasoning:** Maximum height for transaction/service counters for accessibility.
- **Constraint:** Max: 36 inches (0.91m)

- **Rule:** Office Corridor Width
- **Value:** 1.12 m
- **Reasoning:** Minimum width for egress capacity < 50 people.
- **Constraint:** Min: 44 inches (1.12m)

- **Rule:** Restaurant Booth Width
- **Value:** 1.2 m - 1.8 m
- **Reasoning:** Standard booth lengths for 2-4 people.
- **Constraint:** Typical: 48-72 inches

- **Rule:** Theater Wheelchair Space (Front/Rear Entry)
- **Value:** 0.91 m W x 1.22 m D
- **Reasoning:** Floor space for wheelchair seating in assembly areas.
- **Constraint:** Min: 36" x 48" (0.91x1.22m)

## Outdoor & Landscape

- **Rule:** Standard Parking Space
- **Value:** 2.74 m x 5.49 m
- **Reasoning:** Standard dimension for passenger vehicles.
- **Constraint:** Typical: 9x18 feet (2.74x5.49m)

- **Rule:** ADA Parking Space Width
- **Value:** 2.44 m
- **Reasoning:** Minimum width for the vehicle space itself (plus aisle).
- **Constraint:** Strict Min: 8 feet (2.44m)

- **Rule:** ADA Access Aisle Width
- **Value:** 1.52 m
- **Reasoning:** Clear space beside parking spot for loading/unloading.
- **Constraint:** Strict Min: 5 feet (1.52m)

- **Rule:** Two-Way Drive Lane Width
- **Value:** 7.32 m
- **Reasoning:** Allows two vehicles to pass safely (12' per lane).
- **Constraint:** Min: 24 feet (7.32m)

- **Rule:** Public Sidewalk Width
- **Value:** 1.52 m
- **Reasoning:** Preferred width allows two adults to walk side-by-side.
- **Constraint:** Recommended: 5 feet (1.52m)

- **Rule:** Residential Driveway Width (Single)
- **Value:** 3.05 m - 3.66 m
- **Reasoning:** Minimum comfort width for parking and exiting vehicle.
- **Constraint:** Min: 10-12 feet (3.05-3.66m)

- **Rule:** Residential Driveway Width (Double)
- **Value:** 6.10 m - 7.32 m
- **Reasoning:** Allows two cars side-by-side with door opening space.
- **Constraint:** Min: 20-24 feet (6.1-7.3m)

- **Rule:** Retaining Wall Max Height (No Permit)
- **Value:** 0.91 m - 1.22 m
- **Reasoning:** Structural threshold often requiring engineering/permit above this.
- **Constraint:** Limit: 3-4 feet (0.9-1.2m)

- **Rule:** Patio Table Space Per Person
- **Value:** 0.76 m x 0.76 m
- **Reasoning:** Area allowance for outdoor dining comfort.
- **Constraint:** Recommended Area

## Sports & Recreation

- **Rule:** Tennis Court Playing Dimensions
- **Value:** 23.77 m L x 10.97 m W
- **Reasoning:** Regulation size for doubles match (Singles width: 8.23m).
- **Constraint:** Fixed Standard

- **Rule:** Basketball Hoop Height
- **Value:** 3.05 m
- **Reasoning:** Regulation rim height for ages 12+.
- **Constraint:** Standard: 10 feet (3.05m)

- **Rule:** Pool Table Room Clearance
- **Value:** 1.52 m
- **Reasoning:** Minimum cueing clearance around all sides (5ft).
- **Constraint:** Min: 5 feet (1.52m)

- **Rule:** Bowling Lane Length
- **Value:** 18.29 m
- **Reasoning:** Distance from foul line to headpin.
- **Constraint:** Standard: 60 feet (18.29m)

- **Rule:** Bowling Lane Width
- **Value:** 1.05 m
- **Reasoning:** Regulation lane width (excluding gutters).
- **Constraint:** Standard: 41.5 inches (1.05m)

- **Rule:** Treadmill Rear Safety Clearance
- **Value:** 1.52 m - 1.98 m
- **Reasoning:** Area to prevent injury if user falls backward.
- **Constraint:** Min: 5 feet (1.52m)

- **Rule:** Treadmill Side Clearance
- **Value:** 0.60 m
- **Reasoning:** Space for safe mounting/dismounting.
- **Constraint:** Min: 2 feet (0.6m)

- **Rule:** Ping Pong Table Clearance
- **Value:** 1.52 m (Ends), 0.91 m (Sides)
- **Reasoning:** Play area for shots away from table.
- **Constraint:** Recommended

- **Rule:** Swimming Lane Width (Competition)
- **Value:** 2.5 m
- **Reasoning:** FINA standard for Olympic pools.
- **Constraint:** Standard: 2.5m

- **Rule:** Golf Simulator Room Height
- **Value:** 2.74 m - 3.05 m
- **Reasoning:** Clearance for full driver swing arc.
- **Constraint:** Min: 9-10 feet (2.7-3.0m)

## Education & Libraries

- **Rule:** Classroom Desk Clearance (Rows)
- **Value:** 0.76 m - 0.91 m
- **Reasoning:** Circulation space between rows of desks.
- **Constraint:** Min: 30-36 inches (0.76-0.91m)

- **Rule:** Whiteboard Mounting Height
- **Value:** 0.86 m - 0.91 m (Child) / 1.2 m (Adult)
- **Reasoning:** Distance from floor to tray based on user age.
- **Constraint:** Age Dependent

- **Rule:** ADA Library Aisle Width
- **Value:** 0.91 m (Min), 1.07 m (Preferred)
- **Reasoning:** Clearance between book stacks for wheelchair access.
- **Constraint:** Strict Min: 36 inches (0.91m)

- **Rule:** School Locker Dimensions
- **Value:** 0.30 m W x 0.30 m D x 1.52 m H
- **Reasoning:** Standard full-tier locker size.
- **Constraint:** Standard: 12x12x60 inches

- **Rule:** Auditorium Seating Spacing
- **Value:** 0.84 m - 0.91 m (Back-to-Back)
- **Reasoning:** Standard row depth for comfort.
- **Constraint:** Min: 33 inches (0.84m)

- **Rule:** Classroom Chair Clearance
- **Value:** 0.91 m
- **Reasoning:** Space to back out chair and access behind.
- **Constraint:** Min: 36 inches (0.91m)

- **Rule:** Teacher Podium Clearance
- **Value:** 1.22 m
- **Reasoning:** Action zone for teaching movement.
- **Constraint:** Recommended: 48 inches (1.22m)

- **Rule:** Cafeteria Table Spacing
- **Value:** 1.52 m
- **Reasoning:** Main traffic aisle between banks of tables.
- **Constraint:** Min: 60 inches (1.52m)

- **Rule:** Study Carrel Dimensions
- **Value:** 0.91 m W x 0.61 m D
- **Reasoning:** Standard individual quiet study space.
- **Constraint:** Typical: 36x24 inches

- **Rule:** Kindergarten Chair Height
- **Value:** 0.25 m - 0.35 m
- **Reasoning:** Seat height appropriate for 4-6 year olds.
- **Constraint:** Range: 10-14 inches

## Retail & Commercial

- **Rule:** Retail Aisle Width (Accessible)
- **Value:** 0.91 m (minimum), 1.22 m (recommended)
- **Reasoning:** ADA minimum for wheelchair passage. 48" allows two-way traffic.
- **Constraint:** Min: 36 inches (0.91m)

- **Rule:** Retail Service Counter Height (Accessible Portion)
- **Value:** 0.91 m (max)
- **Reasoning:** Maximum height for ADA service transaction counter.
- **Constraint:** Max: 36 inches (0.91m)

- **Rule:** Fitting Room Minimum Dimensions (Accessible)
- **Value:** 1.52 m x 1.52 m (plus turning space)
- **Reasoning:** Space for wheelchair turning (60" diameter).
- **Constraint:** Min: 60x60 inches

- **Rule:** Merchandise Shelf Height (Max Reach)
- **Value:** 1.22 m
- **Reasoning:** maximum reach range for seated unobstructed forward reach.
- **Constraint:** Max: 48 inches (1.22m)

- **Rule:** Main Aisle Width (Supermarket)
- **Value:** 1.83 m - 2.44 m
- **Reasoning:** Allows two carts to pass comfortably with browsing space.
- **Constraint:** Recommended: 6-8 feet

## Healthcare & Accessibility

- **Rule:** Hospital Corridor Width (Patient Areas)
- **Value:** 2.44 m
- **Reasoning:** Minimum width for moving hospital beds and gurneys (Group I-2).
- **Constraint:** Min: 96 inches (2.44m)

- **Rule:** Patient Room Door Width
- **Value:** 1.05 m - 1.22 m
- **Reasoning:** Clearance for hospital bed passage (often unequal leaf doors).
- **Constraint:** Min: 41.5 inches (1.05m)

- **Rule:** ADA Bathroom Turning Radius
- **Value:** 1.52 m
- **Reasoning:** Standard circular turning space for wheelchairs.
- **Constraint:** Diameter: 60 inches (1.52m)

- **Rule:** Hospital Room Clearance (Bed Side)
- **Value:** 1.22 m
- **Reasoning:** Space for medical staff and equipment access.
- **Constraint:** Min: 48 inches (1.22m)

- **Rule:** Accessible Sink Knee Clearance
- **Value:** 0.69 m H x 0.76 m W
- **Reasoning:** Minimum space for knees under lavatory.
- **Constraint:** Min: 27" H x 30" W

## Hospitality & Dining

- **Rule:** Hotel Room Minimum Width
- **Value:** 3.05 m - 3.66 m
- **Reasoning:** Functional width for bed + clearance + furniture.
- **Constraint:** Recommended Min: 10-12 feet

- **Rule:** Hotel Corridor Width
- **Value:** 1.52 m
- **Reasoning:** Allows luggage carts and guests to pass.
- **Constraint:** Min: 5 feet (1.52m)

- **Rule:** Bar Counter Height (Commercial)
- **Value:** 1.07 m
- **Reasoning:** Standard standing bar height.
- **Constraint:** Standard: 42 inches (1.07m)

- **Rule:** Bar Stool Spacing (Center-to-Center)
- **Value:** 0.61 m - 0.76 m
- **Reasoning:** Personal space and elbow room for patrons.
- **Constraint:** Min: 24 inches (0.61m)

- **Rule:** Dining Chair Push-back Clearance
- **Value:** 0.91 m
- **Reasoning:** Space to leave table without obstructing aisle.
- **Constraint:** Min: 36 inches (0.91m)

## Public Facilities & Restrooms

- **Rule:** Standard Stall Width
- **Value:** 1.52 m
- **Reasoning:** Minimum width for wheelchair accessible stall.
- **Constraint:** Min: 60 inches (1.52m)

- **Rule:** Urinal Rim Height (ADA)
- **Value:** 0.43 m (max)
- **Reasoning:** Max height above floor for accessible usage.
- **Constraint:** Max: 17 inches (0.43m)

- **Rule:** Lavatory Rim Height
- **Value:** 0.86 m (max)
- **Reasoning:** ADA maximum height for sink rim or counter.
- **Constraint:** Max: 34 inches (0.86m)

- **Rule:** Toilet Seat Height (ADA)
- **Value:** 0.43 m - 0.48 m
- **Reasoning:** Height range for accessible water closets.
- **Constraint:** Range: 17-19 inches

- **Rule:** Grab Bar Height
- **Value:** 0.84 m - 0.91 m
- **Reasoning:** Height range for horizontal grab bars.
- **Constraint:** Range: 33-36 inches

## Healthcare & Hospitality

- **Rule:** Hospital Corridor Width (Patient)
- **Value:** 2.44 m
- **Reasoning:** Width for two gurneys to pass.
- **Constraint:** Min: 8 feet (2.44m)

- **Rule:** Exam Room Dimensions
- **Value:** 3.05 m x 3.05 m
- **Reasoning:** Space for bed, doctor, assistant, and sink.
- **Constraint:** Min: 10x10 feet

- **Rule:** Patient Bed Clearance (Sides)
- **Value:** 0.91 m
- **Reasoning:** Space for staff access and equipment movement.
- **Constraint:** Min: 36 inches (0.91m)

- **Rule:** Dental Chair Clearance
- **Value:** 0.81 m
- **Reasoning:** Working radius for dentist and hygienist.
- **Constraint:** Min: 32 inches (0.81m)

- **Rule:** Hotel Front Desk Height (ADA Portion)
- **Value:** 0.86 m
- **Reasoning:** Maximum height for wheelchair user transaction.
- **Constraint:** Max: 36 inches (0.86m)

- **Rule:** Hotel Corridor Width
- **Value:** 1.52 m
- **Reasoning:** Standard commercial hallway for luggage carts.
- **Constraint:** Min: 60 inches (1.52m)

- **Rule:** Luggage Cart Width Allowance
- **Value:** 0.91 m
- **Reasoning:** Minimum clear door width for bellhop carts.
- **Constraint:** Min: 36 inches (0.91m)

- **Rule:** Restaurant Bar Stool Spacing
- **Value:** 0.61 m - 0.76 m
- **Reasoning:** Center-to-center spacing for elbow room.
- **Constraint:** Min: 24-30 inches

- **Rule:** Restaurant Booth Seat Depth
- **Value:** 0.41 m - 0.46 m
- **Reasoning:** Standard comfortable seat depth.
- **Constraint:** Range: 16-18 inches

- **Rule:** Commercial Kitchen Work Aisle
- **Value:** 1.07 m
- **Reasoning:** Space for one cook and traffic behind.
- **Constraint:** Min: 42 inches (1.07m)

## Infrastructure & Transportation

- **Rule:** Bicycle Lane Width (Protected)
- **Value:** 2.0 m - 2.1 m
- **Reasoning:** NACTO standard for one-way protected lane.
- **Constraint:** Recommended: 6.5-7 feet

- **Rule:** Bus Stop Shelter Dimensions
- **Value:** 1.22 m D x 2.44 m W
- **Reasoning:** Typical shelter footprint for urban sidewalk.
- **Constraint:** Typical: 4x8 feet

- **Rule:** RV Garage Door Height
- **Value:** 3.66 m - 4.27 m
- **Reasoning:** Clearance for roof AC units on large RVs.
- **Constraint:** Min: 12-14 feet

- **Rule:** Fire Engine Access Road Width
- **Value:** 6.10 m
- **Reasoning:** Minimum unobstructed width for apparatus.
- **Constraint:** Strict Min: 20 feet (6.1m)

- **Rule:** Vertical Clearance for Fire Apparatus
- **Value:** 4.11 m
- **Reasoning:** Clear height for ladders and equipment.
- **Constraint:** Strict Min: 13ft 6in (4.11m)

- **Rule:** Sidewalk Clear Height (Tree Limbs)
- **Value:** 2.44 m
- **Reasoning:** Clearance for pedestrians and cyclists.
- **Constraint:** Min: 8 feet (2.44m)

- **Rule:** EV Charging Station Width
- **Value:** 2.74 m
- **Reasoning:** Parking width plus equipment access.
- **Constraint:** Min: 9 feet (2.74m)

- **Rule:** Parking Wheel Stop Length
- **Value:** 1.83 m
- **Reasoning:** Standard length for concrete or rubber stop.
- **Constraint:** Standard: 6 feet (1.83m)

- **Rule:** Drive-Thru Lane Width
- **Value:** 3.05 m
- **Reasoning:** Minimum width for vehicle straight-aways.
- **Constraint:** Min: 10 feet (3.05m)

- **Rule:** Dumpster Enclosure Interior
- **Value:** 3.05 m W x 3.05 m D
- **Reasoning:** Minimum for single dumpster + maneuvering.
- **Constraint:** Min: 10x10 feet
