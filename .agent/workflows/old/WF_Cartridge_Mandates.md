---
description: Definitive mandates for Cartridge Script Structure, UV Unwrapping, and Edge Logic.
---

# AGENT: CARTRIDGE_MANDATES
...
class MASSA_OT_CartridgeName(Massa_OT_Base):
    ...
    def get_slot_meta(self):
        ...
        return {
            ...
            # Ensure Slot 9 is defined if sockets are used
 ...
 # Use 'self.prop_name' to access parameters
        ...
        # Mandate: Assign material_index to faces.
        # 0=Base, 1=Detail, 2=Trim, ... 9=Socket
        for f in bm.faces:
            f.material_index = 0
        ...
        for e in bm.edges:
            if e.is_boundary:
                ...
                e.seam = True     # LOGIC BRIDGE: Perimeter is also a UV Seam
        
        # ---------------------------------------------------------
        # PHASE 4: UV (Unwrap Strategy)
        # ---------------------------------------------------------

        # Mandate: All faces must be mapped to avoid "Zero Area" errors.
        # Strategies: BOX, UNWRAP (Seam-based), FIT, STRIP.
        # AUDIT RULE: Margins be > 0.001 to prevent bleeding.
...
| Strategy | Trigger | Implementation | Audit Requirement |
| :--- | :--- | :--- | :--- |
| **BOX** | Hard Surface / Blocky | `bmesh.ops.uv_project` | **Scale** must match object size. |
| **UNWRAP** | Organic / Curved | `bmesh.ops.uv_unwrap` | **Seams** must be marked in Phase 3. **Margin > 0.02**. |
| **FIT** | Glass / Screens | Bounds fit to 0-1 space. | Clamp to **0.01 - 0.99** (No Touching Edges). |
| **STRIP** | Pipes / Trims | Follow active quads (Grid Fill). | Requires **Segmentation** (Cap Cut + Rail Cut). |

### 📝 Implementation Rules

1. **Seam Dependency**:
    * If using `UNWRAP`, you **MUST** define seams in Phase 3.
    * **Logic Bridge**: If assigning Edge Slot `3` (GUIDE) or `1` (PERIMETER), you must also set `e.seam = True`.
...
| **4** | **DETAIL** | Surface Detail | Bevel Only (No Sharp) |
| **5** | **FOLD** | Cloth Wrinkle/Tension | Cloth Pinning / subdiv Crease |
